...
2. **Monorepo detection** — CRITICAL for all subsequent steps:
   - Check for apps/, packages/, libs/, modules/ directories
   - Check for workspace config: workspaces in package.json, pnpm-workspace.yaml, nx.json, turbo.json, lerna.json
   - If monorepo: list each app/package with its own config files
   - Report: "single app" or "monorepo with [N] apps/packages"

3. **Source code inventory**: file counts by extension, main source directories, module organization

4. **Configuration files**: root-level configs, environment configs, .gitignore patterns

5. **Dependencies**: direct count, dev count, lock file presence
...
## Structure: [single app | monorepo — N apps/packages]
## Workspace Config: [nx/turbo/lerna/pnpm/yarn/none]
